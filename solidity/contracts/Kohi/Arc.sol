...
library ArcMethods {
    function create(
        ...
        int64 averageRadius = Fix64V1.div(
            Fix64V1.add(Fix64V1.abs(data.radius.x), Fix64V1.abs(data.radius.y)),
            Fix64V1.TWO
        );

        data.deltaAngle = Fix64V1.mul(
            Trig256.acos(
                Fix64V1.div(
                    averageRadius,
                    Fix64V1.add(
        ...
        while (angle < data.startAngle)
            angle = Fix64V1.add(angle, Fix64V1.TWO_PI);

        data.angle = angle;

        data.steps = (int32)(
            Fix64V1.div(
                Fix64V1.sub(data.angle, data.startAngle),
                data.deltaAngle
            ) / Fix64V1.ONE
        );
    }
        ...
        returns (VertexData[] memory results)
    {
...

        {
            vertexData.position = Vector2(
                Fix64V1.add(
                    data.origin.x,
                    Fix64V1.mul(Trig256.cos(data.startAngle), data.radius.x)
                ),
                Fix64V1.add(
                    data.origin.y,
                    Fix64V1.mul(Trig256.sin(data.startAngle), data.radius.y)
                )
            );
        ...
        for (uint32 i = 0; i <= uint32(data.steps); i++) {
            if (angle < data.angle) {
                vertexData.position = Vector2(
                    Fix64V1.add(
                        data.origin.x,
                        Fix64V1.mul(Trig256.cos(angle), data.radius.x)
                    ),
                    Fix64V1.add(
                        data.origin.y,
                        Fix64V1.mul(Trig256.sin(angle), data.radius.y)
                    )
                );

                results[1 + i] = vertexData;

                angle = Fix64V1.add(angle, data.deltaAngle);
            }
        }
