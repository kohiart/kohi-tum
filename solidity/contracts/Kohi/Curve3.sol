...
library Curve3 {
    ...
    function recursiveBezier(
        ...
        a.dx = Fix64V1.sub(f.x3, f.x1);
        a.dy = Fix64V1.sub(f.y3, f.y1);

        int64 d = Fix64V1.abs(
            Fix64V1.sub(
                Fix64V1.mul(Fix64V1.sub(f.x2, f.x3), a.dy),
                Fix64V1.mul((Fix64V1.sub(f.y2, f.y3)), a.dx)
            )
        );
        ...
        if (d > MathUtils.Epsilon) {
            if (
                ...
                Fix64V1.mul(
                    ...
                    Fix64V1.mul(a.dx, a.dx) + Fix64V1.mul(a.dy, a.dy)
                )
            ) {
                ...
                if (da >= Fix64V1.PI) {
                    da = Fix64V1.sub(Fix64V1.TWO_PI, da);
                }
            ...
            da = Fix64V1.add(Fix64V1.mul(a.dx, a.dx), Fix64V1.mul(a.dy, a.dy));

            if (da == 0) {
                d = MathUtils.calcSquareDistance(f.x1, f.y1, f.x2, f.y2);
            } else {
                d = Fix64V1.add(
                    Fix64V1.mul(Fix64V1.sub(f.x2, f.x1), a.dx),
                    Fix64V1.div(Fix64V1.mul(Fix64V1.sub(f.y2, f.y1), a.dy), da)
                );

                if (d > 0 && d < Fix64V1.ONE) {
                    return;
                }

                if (d <= 0) {
                    d = MathUtils.calcSquareDistance(f.x2, f.y2, f.x1, f.y1);
                } else if (d > Fix64V1.ONE) // *** was d >= 1f
                {
                    ...
                    d = MathUtils.calcSquareDistance(
                        f.x2,
                        f.y2,
                        Fix64V1.add(f.x1, Fix64V1.mul(d, a.dx)),
                        Fix64V1.add(f.y1, Fix64V1.mul(d, a.dy))
                    );
                }
            }
        ...
        recursiveBezier(self, RecursiveBezier(f.x1, f.y1, a.x12, a.y12, a.x123, a.y123, f.level + 1));
        recursiveBezier(self, RecursiveBezier(a.x123, a.y123, a.x23, a.y23, f.x3, f.y3, f.level + 1));
    }
}
